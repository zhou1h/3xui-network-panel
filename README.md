...
- 多服务器资源管理、连通性检查及节点/客户端/SOCKS5 数量统计
- VLESS + Reality + RAW（Xray 内部值为 `tcp`）入口
- 客户端和服务端统一使用 `xtls-rprx-vision`
- SOCKS5 单节点入口与二维码分享
- 多跳链路编排和 Xray 路由模板
- 调用各节点 3x-ui 自带的 Reality 扫描器，在节点本机探测并优选目标
...
- 目标：创建 Reality 单节点时，由目标节点调用 3x-ui 的 Reality 扫描 API，筛选 TLS 1.3、HTTP/2、X25519 和证书有效的目标，并按节点实测延迟选择；旧版 3x-ui 不支持扫描时回退到兼容候选
- 维护：每个 VLESS 单节点都可执行“节点扫描优选”“切换下一个备用”或手动设置 SNI/Target；维护目标不会更换 UUID、Reality 密钥或 Short ID
- 重启：入站、客户端和路由全部写入后，先重启 Xray，再重启 3x-ui 面板

自动目标的延迟由“承载该入口的节点服务器”测得，因此不同节点可能选择不同目标。客户端到节点的网络质量仍需独立测试。资源中心的节点、客户端和 SOCKS5 数量会在全量健康检查或单点检测后刷新。
...
先在单节点入口执行“节点扫描优选”，重新导入更新后的分享链接。再确认客户端完整支持 `xtls-rprx-vision`，分享链接中包含 `flow=xtls-rprx-vision`，服务端和客户端的 SNI、公钥、Short ID、端口一致，并检查云防火墙与系统防火墙。

“节点扫描优选”需要目标节点的 3x-ui 提供 `/panel/api/server/scanRealityTargets`。如果接口不可用，请升级 3x-ui；升级前仍可使用面板内的目标预设或手动 SNI/Target。
